    ...
    with ctx.generate.draft() as draft:
        ...
        for biome in biomes:
            ...
            data["features"] = [
                ...
                [],
                [],
                geode_purge(draft, bottom, top, "initial"),
                [],
                [],
                [],
            ...
            if MAJOR_VERSION != "":  # type: ignore
                ...
                draft.overlays[f"skyvoid_{MAJOR_VERSION}"].data[
                    f"{namespace}:{name}"
                ] = WorldgenBiome(overlay_data)
...
def get_dimension_bounds(zip: ZipFile, overlay_name: str, dimension: str):
    filename = (
        f"data/minecraft/dimension_type/overworld.json"
        if dimension == "overworld"
        else f"data/minecraft/dimension_type/the_{dimension}.json"
    )
    try:
        file = zip.open(f"{overlay_name}/{filename}")
        data = json.loads(file.read())
...
def get_biomes_in_tag(zip: ZipFile, overlay_name: str, namespace: str, name: str):
    ...
    for biome in biomes:
        if biome.startswith("#"):
            ...
            modified_biomes.extend(
                get_biomes_in_tag(zip, overlay_name, namespace, name)
            )
        else:
...
def create_noise_setting_patch(
    ctx: Context,
    cache_loc: str,
    url: str,
    dimension: str = "overworld",
