...
import requests
from beet import BlockTag, Context, Draft
...
def gen(
    ctx: Context,
    cache_loc: str,
    ignored_dimensions: list[str] = [],
    full_removal: bool = False,
    remove_structures: bool = True,
    keep_spawners: bool = False,
):
    ...
    with ctx.generate.draft() as draft:
        ...
        if not remove_structures:
            ...
            if "water" in initial_purge:
                initial_purge.remove("water")  # type: ignore
            if "minecraft:water" in initial_purge:
                initial_purge.remove("minecraft:water")  # type: ignore
            if "lava" in initial_purge:
                initial_purge.remove("lava")  # type: ignore
            if "minecraft:lava" in initial_purge:
                initial_purge.remove("minecraft:lava")  # type: ignore
            draft.data[f"{NAME}:initial_purge"] = BlockTag({"values": initial_purge})

        if "jigsaw" in final_purge:
            final_purge.remove("jigsaw")  # type: ignore
        if "minecraft:jigsaw" in final_purge:
            final_purge.remove("minecraft:jigsaw")  # type: ignore
            ...
            if name in NETHER_BIOMES:
                if clear_nether:
                    data["features"] = [
                        ...
                        else geode_purge(draft, 0, 256, "initial"),
                        [],
                        [],
                        [],
                        [],
                        geode_purge(draft, 0, 256, "initial")
                        if remove_structures
                        else [],
                        [],
                        [],
                ...
                data["features"] = [
                    ...
                    []
                    if remove_structures
                    else geode_purge(draft, -64, 320, "initial"),
                    spawners,
                    [],
                    [],
                    [],
                    geode_purge(draft, -64, 320, "initial")
                    if remove_structures
                    else [],
                    [],
                    [],
                    [],
                    flowers,
                    geode_purge(draft, -64, 320, "final"),
                ]
...
def geode_purge(draft: Draft, first: int, last: int, phase: str):
    ...
    for i in range(first, last, 16):
        ...
        data = {
            ...
            "placement": [
                {"type": "minecraft:height_range", "height": {"absolute": i}}
            ],
        }
